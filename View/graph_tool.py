...
from matplotlib.backends.backend_qt5 import TimerQT, SPECIAL_KEYS, _MODIFIER_KEYS, cursord

from PySide6.QtCore import Qt, QObject, QUrl, QRectF, QTimer, QSize, Signal, Property
from PySide6.QtGui import QPen, QImage
from PySide6.QtWidgets import QApplication
from PySide6.QtQuick import QQuickPaintedItem, QQuickImageProvider

class FigureCanvasQtQuick(QQuickPaintedItem, FigureCanvasBase):
    ...
    dpi_ratio_changed = Signal()

    # map Qt button codes to MouseEvent's ones:
    buttond = {Qt.LeftButton: MouseButton.LEFT,
    ...
    dpi_ratio = Property(float, get_dpi_ratio, set_dpi_ratio, notify=dpi_ratio_changed)
    ...
    def drawRectangle(self, rect):
        ...
        if rect is not None:
            def _draw_rect_callback(painter):
                pen = QPen(Qt.black, 1 / self.dpi_ratio,
                                 Qt.DotLine)
                painter.setPen(pen)
    ...
    def geometryChanged(self, new_geometry, old_geometry):
        ...
        self.draw_idle()
        QQuickPaintedItem.geometryChanged(self,
                                                  new_geometry,
                                                  old_geometry)
...
class MatplotlibIconProvider(QQuickImageProvider):
    """ This class provide the matplotlib icons for the navigation toolbar.
    """

    def __init__(self, img_type=QQuickImageProvider.Image):
        self.basedir = os.path.join(matplotlib.rcParams['datapath'], 'images')
        QQuickImageProvider.__init__(self, img_type)

    def requestImage(self, ids, size, reqSize):
        img = QImage(os.path.join(self.basedir, ids + '.png'))
        size.setWidth(img.width())
        size.setHeight(img.height())
        return img


class NavigationToolbar2QtQuick(QObject, NavigationToolbar2):
    ...
    wspaceChanged = Signal()
    hspaceChanged = Signal()
    ...
    left = Property(float, getLeft, setLeft, notify=leftChanged)
    ...
    right = Property(float, getRight, setRight, notify=rightChanged)
    ...
    top = Property(float, getTop, setTop, notify=topChanged)
    ...
    bottom = Property(float, getBottom, setBottom, notify=bottomChanged)
    ...
    hspace = Property(float, getHspace, setHspace, notify=hspaceChanged)
    ...
    wspace = Property(float, getWspace, setWspace, notify=wspaceChanged)
